...
INSERT INTO blogs (
  id, title, slug, excerpt, content, cover_image, category, author, status, reading_time, published_at
)
VALUES (
  $blog1id$uploaded_vitamin-d-the-sunshine-vitamin$blog1id$,
  $blog1t$Vitamin D: The Sunshine Vitamin$blog1t$,
  $blog1s$vitamin-d-the-sunshine-vitamin$blog1s$,
  $blog1e$Vitamin D is a unique vitamin that most people don’t get enough of.$blog1e$,
  $blog1c$the sunshine vitamin.”
...
Homoeopathic approach to vit D deficiency:$blog1c$,
  NULL,
  $blog1cat$Vitamins$blog1cat$,
  'Dr. Varsha Bandi',
  'published',
  3,
  NOW()
...
Normally, vitamin B2 is considered safe. An overdose is unlikely, as the body can absorb up to around 27 milligrams Trusted Source of riboflavin, and it expels any additional amounts in the urine.$blog2c$,$blog2cat$Vitamins$blog2cat$,'Dr. Varsha Bandi','published',4,NOW())
ON CONFLICT (slug) DO NOTHING;
...
Thiamine supplements are also used to treat people going through major alcohol withdrawal.$blog3c$,$blog3cat$Vitamins$blog3cat$,'Dr. Varsha Bandi','published',4,NOW())
ON CONFLICT (slug) DO NOTHING;
...
Many flavoured yogurts contain significant amounts of added sugar. Plain curd is generally the healthier everyday option.$blog5c$,$blog5cat$Nutrition$blog5cat$,'Dr. Varsha Bandi','published',4,NOW())
ON CONFLICT (slug) DO NOTHING;
...
Athletes White Rice or Basmati around exercise; Brown Rice at other meals$blog6c$,$blog6cat$Nutrition$blog6cat$,'Dr. Varsha Bandi','published',5,NOW())
ON CONFLICT (slug) DO NOTHING;
...
Fact: These products may differ in production methods and flavour, but there is insufficient evidence that they consistently provide superior health outcomes compared with good-quality regular cow ghee or grass-fed ghee.$blog7c$,$blog7cat$Nutrition$blog7cat$,'Dr. Varsha Bandi','published',5,NOW())
ON CONFLICT (slug) DO NOTHING;
...
There is no universal scientific rule that banana and milk cannot be consumed together. The combination can be nutritious, although some individuals may experience digestive discomfort.$blog8c$,$blog8cat$Nutrition$blog8cat$,'Dr. Varsha Bandi','published',6,NOW())
ON CONFLICT (slug) DO NOTHING;
...
• For high-risk individuals, steam or cook sprouts before eating.$blog9c$,$blog9cat$Nutrition$blog9cat$,'Dr. Varsha Bandi','published',6,NOW())
ON CONFLICT (slug) DO NOTHING;
